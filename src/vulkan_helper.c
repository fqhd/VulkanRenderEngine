...
void create_descriptor_pool(Vulkan *v)
{
    ...
    poolSize.descriptorCount = v->num_image_views;

    VkDescriptorPoolCreateInfo poolInfo = {0};
    poolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    poolInfo.poolSizeCount = 1;
    poolInfo.pPoolSizes = &poolSize;
    poolInfo.maxSets = v->num_image_views;

    if (vkCreateDescriptorPool(v->logical_device, &poolInfo, NULL, &v->descriptorPool) != VK_SUCCESS)
    {
...
void create_descriptor_sets(Vulkan *v)
{
    ...
    for (size_t i = 0; i < v->num_image_views; i++)
    {
        ...
        vkUpdateDescriptorSets(v->logical_device, 1, &descriptorWrite, 0, NULL);
    }
}
