...
private:
	...
	VkPipelineLayout pipelineLayout; // This is for shader uniforms
	...
	void createFramebuffers(){
		// Creating a frame buffer for each image in the swapchain
	...
	void createSwapChain() {
		// Gathering information about supported swapchain properties
		...
		// Filling the swapchain create info structure
		VkSwapchainCreateInfoKHR createInfo{};
		createInfo.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR;
		createInfo.surface = surface; // Specifies which surface the swapchain should be tied to 
		createInfo.minImageCount = imageCount;
		createInfo.imageFormat = surfaceFormat.format;
		createInfo.imageColorSpace = surfaceFormat.colorSpace;
		createInfo.imageExtent = extent;
		createInfo.imageArrayLayers = 1; // Number of layers per image always 1 unless you are doing 3D stuff
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
		...
		for (const auto& queueFamily : queueFamilies) {
			...
			vkGetPhysicalDeviceSurfaceSupportKHR(device, i, surface, &isSupported);
			if(isSupported){
	...
	void createLogicalDevice(){
		...
		// Setting up device extensions
		char** extensions = (char**)malloc(sizeof(char*)*2);

		const char* extension_name = "VK_KHR_portability_subset";
		const char* extension_name_2 = "VK_KHR_swapchain";

		*extensions = (char*)malloc(strlen(extension_name) + 1);
		*(extensions + 1) = (char*)malloc(strlen(extension_name_2) + 1);

		memcpy(*extensions, extension_name, strlen(extension_name) + 1);
		memcpy(*(extensions + 1), extension_name_2, strlen(extension_name_2) + 1);
		...
		// Retrieving the queue from the device.
	...
	void pickPhysicalDevice(){
		...
		if(!deviceCount){
			std::cout << "There are no vulkan supported devices" << std::endl;
		}else{
			std::cout << "There are vulkan supported devices" << std::endl;
		}

		printf("Found %u devices in the system\n", deviceCount);

		for(int i = 0; i < deviceCount; i++){
			if(isDeviceSuitable(physicalDevices[i])){
				physicalDevice = physicalDevices[i];
				std::cout << "Found a device with graphics family" << std::endl;
				return;
			}
		}
	}
	...
	bool isDeviceSuitable(VkPhysicalDevice device) {
		...
		bool f = indices.isComplete() && swapChainAdequate;
		return f;
	}
